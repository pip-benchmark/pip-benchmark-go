...
import (
	...
	benchenv "github.com/pip-benchmark/pip-benchmark-go/runner/environment"
...
type ReportGenerator struct {
	...
	results       *benchres.ResultsManager
	parameters    *benchparam.ParametersManager
	benchmarks    *bench.BenchmarksManager
	environment   *benchenv.EnvironmentManager
}

func NewReportGenerator(configuration *benchconf.ConfigurationManager,
	results *benchres.ResultsManager, parameters *benchparam.ParametersManager,
	benchmarks *bench.BenchmarksManager, environment *benchenv.EnvironmentManager) *ReportGenerator {
...
func (c *ReportGenerator) generateSystemBenchmark() string {
	...
	output += c.newLine
	output += fmt.Sprintf("  CPU Performance (MFLOP/s): %d",
		util.Formatter.FormatNumber(c.environment.CpuMeasurement(), 2))
	output += c.newLine
	output += fmt.Sprintf("  Video Performance (GOP/s): %d",
		util.Formatter.FormatNumber(c.environment.VideoMeasurement(), 2))
	output += c.newLine
	output += fmt.Sprintf("  Disk Performance (MB/s):   %d",
